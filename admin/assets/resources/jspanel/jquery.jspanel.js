...
if (!$.fn.jquery || !$.fn.uniqueId || !$.widget || !$.ui.mouse || !$.ui.draggable || !$.ui.resizable) {
    ...
    console.log("Loaded: jQuery " + $.fn.jquery + ", jQuery UI " + $.ui.version + "\nUI core: " + $.isFunction($.fn.uniqueId) + ", UI widget: " + $.isFunction($.widget) + ", UI mouse: " + $.isFunction($.ui.mouse) + ", UI draggable: " + $.isFunction($.ui.draggable) + ", UI resizable: " + $.isFunction($.ui.resizable));
}
var jsPanel = {
    ...
    template: '<div class="jsPanel jsPanel-theme-default jsPanel-state-initialized">' + '<div class="jsPanel-hdr jsPanel-theme-default">' + '<h3 class="jsPanel-title"></h3>' + '<div class="jsPanel-hdr-r">' + '<div class="jsPanel-btn-close"><i class="jsglyph jsglyph-remove"></i></div>' + '<div class="jsPanel-btn-max"><span class="jsglyph jsglyph-maximize"></span></div>' + '<div class="jsPanel-btn-norm"><span class="jsglyph jsglyph-normalize"></span></div>' + '<div class="jsPanel-btn-min"><span class="jsglyph jsglyph-minimize"></span></div>' + '<div class="jsPanel-btn-small"><span class="jsglyph jsglyph-chevron-up"></span></div>' + '<div class="jsPanel-btn-smallrev"><span class="jsglyph jsglyph-chevron-down"></span></div>' + '</div>' + '<div class="jsPanel-hdr-toolbar jsPanel-clearfix"></div>' + '</div>' + '<div class="jsPanel-content jsPanel-theme-default"></div>' + '<div class="jsPanel-ftr jsPanel-theme-default jsPanel-clearfix"></div>' + '</div>',
    // add toolbar
    ...
    configControls: function(panel) {
        ...
        controls.forEach(function(ctrl) {
            if (panel.option.controls[ctrl]) {
                panel.control('disable', ctrl);
            }
        });
    },
    // configure iconfonts
    configIconfont: function(panel) {
        ...
        if (optIconfont === 'bootstrap') {
            ...
            controlsArray.forEach(function(item, i) {
                $('.jsPanel-btn-' + item, controls).empty().append('<i class="fa fa-' + fontawesomeArray[i] + '"></i>');
            });
        }
    },
    // builds toolbar
    ...
    getMaxpanelshift: function(panel) {
        ...
        return {
            maxshiftH: horiz,
            maxshiftV: vert,
            maxCornerH: cornerHoriz,
            maxCornerV: cornerVert
        };
    },
    ...
    movetoMinified: function(panel) {
        ...
        if (panel.status !== "minimized" || panel.option.panelstatus !== "minimized") {
            ...
            panel.css({
                ...
                top: 0,
                opacity: 1
            }).appendTo('#jsPanel-min-container').resizable({
                disabled: true
            }).draggable({
                disabled: true
            });
    ...
    normalize: function(panel) {
        ...
        interactions.forEach(function(action) {
            if (panel.option[action] !== "disabled") {
                ...
                $('.ui-icon-gripsmall-diagonal-se', panel).css({
                    'background-image': 'none',
                    'text-indent': 0
                });
                $('.ui-resizable-handle', panel).css({
                    'cursor': ''
                });
            }
        });
...
(function($) {
    $.jsPanel = function(config) {
        ...
        if (jsP.option.paneltype.type === 'modal') {
            ...
            if (jsP.option.paneltype.cornerBG) {
                ...
                if (jsP.option.paneltype.position === "top") {
                    ...
                    cornerPos = parseInt(jsP.option.size.height) / 2 - 12 + (cornerOY) + "px";
                    corner.css({
                        borderLeftColor: cornerBG,
                        left: jsP.option.size.width,
                        top: cornerPos
                    });
                }
            }
        ...
        if ($.isPlainObject(jsP.option.resizable)) {
            ...
            $('.ui-icon-gripsmall-diagonal-se', jsP).css({
                'background-image': 'none',
                'text-indent': -9999
            });
            $('.ui-resizable-handle', jsP).css({
                'cursor': 'inherit'
            });
        }
        /* option.content ------------------------------------------------------------------------------------------- */
